...
def get_text_messages(message):
    ...
    if cur_menu != None and ms_text in cur_menu.buttons:  # проверим, что команда относится к текущему меню
        ...
        # ======================================= Развлечения
            ...
            bot.send_message(chat_id, text=get_slovo())
            ...
            bot.send_photo(chat_id, photo=get_randomCoffee())
...
def send_help(chat_id):
    global bot
    bot.send_message(chat_id, "Автор: Огородников Илья")
    markup = types.InlineKeyboardMarkup()
    btn1 = types.InlineKeyboardButton(text="Напишите автору")
    markup.add(btn1)
    img = open('Огородников.jpg', 'rb')
...
def get_slovo():
    array_anekdots = []
    req_anek = requests.get('https://calculator888.ru/random-generator/sluchaynoye-slovo')
    if req_anek.status_code == 200:
        soup = bs4.BeautifulSoup(req_anek.text, "html.parser")
